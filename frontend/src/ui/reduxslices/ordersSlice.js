...
export const getAUsersOrders = createAsyncThunk(
	'get/usersOrders',
	async user_id => {
		try {
...
export const emptyUsersOrderItems = createAsyncThunk(
	'removeUsersOrders',
	async () => {
	...
	extraReducers(builder) {
		builder
			.addCase(getOrders.fulfilled, (state, action) => {
				...
				state.orderItems = [
					...state.orderItems.concat(action.payload.orderItems),
				];
			})
			.addCase(getAUsersOrders.fulfilled, (state, action) => {
				state.status = 'succeeded';
				state.usersOrderItems = [
					...state.usersOrderItems.concat(
						action.payload.usersOrderItems
					),
				];
			})
