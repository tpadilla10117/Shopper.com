...
} from '../db/dbadapters/saved_products.js';

import { updateUser } from '../db/dbadapters/admin.js';
...
usersRouter.delete(
	'/:userId/my-account/saved-items/:productid',
	async (req, res, next) => {
		...
		try {
			if (userId === null || typeof userId === undefined || !userId) {
				...
				const removeItem = await deleteSavedProductByProductid(
					productid
				);
	...
	async (req, res, next) => {
		...
		try {
			if (userId === null || typeof userId === undefined || !userId) {
				...
				const usersSavedProducts = await getASavedProductByUserId(
					savedProduct
				);
			...
			next(
				user
					? {
							...
							message:
								'You cannot delete an account that is not yours',
					  }
					: {
