...
import {
	...
	getUserById,
	getUserByUsername,
} from './dbadapters/users.js';

import {
	createUserAddressByUser,
	getUserAddress,
} from './dbadapters/user_addresses.js';
...
} from './dbadapters/saved_products.js';
...
} from './dbadapters/orders.js';

import {
	getAllProductCategories,
	createProductCategories,
} from './dbadapters/product_category.js';

import {
	createShoppingSession,
	retrieveShoppingSessionItemById,
} from './dbadapters/shopping_sessions.js';
...
} from './dbadapters/product_reviews.js';
 ...
 */
		console.log(
			'Here are all a users orders: ',
			await getAllOrdersByAUserId(1)
		);

		console.log(
			'Here are all a users saved items: ',
			await getSavedProductsByUserId(1)
		);
...
async function seedUserAddress() {
	...
	try {
		...
		const userAddress = await Promise.all(
			seedAddress.map(createUserAddressByUser)
		);
...
async function seedInitialProductCategories() {
	...
	try {
		...
		const productCategories = await Promise.all(
			productCategoriesToCreate.map(createProductCategories)
		);
	...
	try {
		const productsToCreate = [
			...
			},
			{
				...
				description:
					'The b-ball OG that puts a fresh spin on what you know best: durably stitched overlays, clean finishes and the perfect amount of flash to make you shine.',
			...
			},
			{
				...
				description:
					"There is never a wrong time to stand tall. Created for the hardwood but taken to the streets, the High Top gives the '80s b-ball icon the perfect lift.",
...
async function seedSavedProducts() {
	...
	try {
		...
		const products = await Promise.all(
			savedProduct.map(createSavedProduct)
		);
...
async function seedProductReviews() {
	...
	try {
		...
		const productReviewData = [
			{
				title: 'Nice Shirt!',
				description:
					'This jacket perfectly fits my style.  It is snug, feels comfortable, and has a great overall texture!  Only giving it 4 stars because it is a bit pricey.',
				rating: 4,
				user_id: user.id,
		...
		const reviews = await Promise.all(
			productReviewData.map(createProductReview)
		);
...
async function seedShoppingSession() {
	try {
		...
		const createdSession = await Promise.all(
			sessionData.map(createShoppingSession)
		);
...
console.log('heyo inside the seed.js file');
